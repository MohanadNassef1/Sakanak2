...
14. NO ID DOCUMENTS: Sakanak never asks users to upload a national ID, passport, or any identity document. If a user asks about ID/passport verification, tell them clearly that no ID or passport upload is required — accounts are confirmed via email and phone and a completed profile. Never request or accept identity documents.`;
    ...
    if (lastUser?.content && userId) {
      supabaseAdmin.from('ai_chat_logs').insert({
        user_id: userId, chat_type: 'finder', session_id: sessionId,
        role: 'user', content: String(lastUser.content).slice(0, 10000), language,
      }).then(({ error }) => { if (error) console.error('log user err', error); });
    }

    // Tee the SSE stream: pass-through to client, accumulate assistant text for logging
    let assistantText = '';
