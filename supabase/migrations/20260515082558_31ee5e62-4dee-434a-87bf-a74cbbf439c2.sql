-- Allow gender to be NULL on profiles
ALTER TABLE public.profiles ALTER COLUMN gender DROP NOT NULL;

-- Update handle_new_user to leave gender NULL when not provided (instead of defaulting to 'male')
...
-- Allow first-time gender set (NULL -> value); still block changes once set, except by admins
...
AS $function$
BEGIN
  IF OLD.gender IS DISTINCT FROM NEW.gender
