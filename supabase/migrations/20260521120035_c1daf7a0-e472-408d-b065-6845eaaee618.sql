...
BEGIN
  ...
  v_is_smoker := COALESCE((NEW.raw_user_meta_data->>'is_smoker')::BOOLEAN, FALSE);
  v_has_pets := COALESCE((NEW.raw_user_meta_data->>'has_pets')::BOOLEAN, FALSE);
  v_pet_type := NULLIF(TRIM(COALESCE(NEW.raw_user_meta_data->>'pet_type', '')), '');
  ...
  BEGIN
    ...
    INSERT INTO public.profiles (
      user_id, email, full_name, gender, nationality, date_of_birth, age,
      referral_code, referred_by, phone, interested_area_1, interested_area_2,
      personality_tags, occupation_status, university, faculty, job_title, hear_about_us,
      is_smoker, has_pets, pet_type
    ) VALUES (
      NEW.id, NEW.email,
      COALESCE(v_full_name, SPLIT_PART(NEW.email, '@', 1)),
      CASE WHEN v_gender IS NOT NULL THEN v_gender::public.user_gender ELSE NULL END,
      v_nationality,
      v_dob::DATE,
      ...
      v_phone, v_area1, v_area2,
      COALESCE(v_personality_tags, ARRAY[]::TEXT[]),
      v_occupation_status, v_university, v_faculty, v_job_title, v_hear_about_us,
      v_is_smoker, v_has_pets, CASE WHEN v_has_pets THEN v_pet_type ELSE NULL END
    )
