...
CREATE OR REPLACE FUNCTION public.get_room_owner_public_info(_owner_id uuid)
 RETURNS TABLE(user_id uuid, full_name text, avatar_url text, gender user_gender, is_smoker boolean, has_pets boolean, pet_type text, occupation text, nationality text, about text, looking_for text, age integer, personality_tags text[], university text, faculty text, job_title text, is_verified boolean)
 LANGUAGE sql
